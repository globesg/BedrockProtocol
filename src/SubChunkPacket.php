...
class SubChunkPacket extends DataPacket implements ClientboundPacket{
	...
	protected function decodePayload(ByteBufferReader $in, int $protocolId) : void{
		...
		$this->baseSubChunkPosition = $protocolId >= ProtocolInfo::PROTOCOL_1_26_40 ? SubChunkPosition::read($in, true) : SubChunkPosition::readVarInts($in);
	...
	protected function encodePayload(ByteBufferWriter $out, int $protocolId) : void{
		...
		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_40){ $this->baseSubChunkPosition->write($out, true); }else{ $this->baseSubChunkPosition->writeVarInts($out); }

		if($protocolId >= ProtocolInfo::PROTOCOL_1_26_40){ VarInt::writeUnsignedInt($out, count($this->entries->getEntries())); }else{ LE::writeUnsignedInt($out, count($this->entries->getEntries())); }
